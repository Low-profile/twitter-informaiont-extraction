...
regxp3 = "((\d+|(" + numbers + "[-\s]?)+) " + hms + "s?)"

time_base = '((([0-1]?[0-9])|(2[0-3])):[0-5][0-9])'
...
reg6 = re.compile(bus_num)
reg7 = re.compile(regxp3)
reg8 = re.compile(time_base)
...
def tag(text):
    ...
    #with open('stops.txt','r') as stop_file:
    #   list_stops = [x.rstrip() for x in stop_file.readlines()]

    with open('routes.txt','r') as routes_file:
        buses = [x.rstrip() for x in routes_file.readlines()]
    ...
    # matching string. Captures expressions such as 'number of minutes' ago, etc.
    ...
    bus_found = [x.strip() for x in found if x.strip() in buses]
    #for bus in bus_found:
        #text = re.sub('(?!<BUS>)' + bus + '(?!</BUS>)', '<BUS>' + bus + '</BUS>', text)

    stop_found = []
    stop_id=[]
    for stop in list_stops:
        #words = [word for word in stop.split(' ') if len(word) > 3]
        founded = False
        #for word in words:
        #    if word in text:
        #        founded = True
        #        text = text.replace(stop +'(?!</stop>)', '<stop>' + stop + '</stop>')
        if stop in text:
            founded = True
        if founded:
            stop_id.append(stop_df.loc[stop_df['stop_name']==stop,'stop_id'].values[0])
            stop_found.append(stop)
 
    return timex_found,bus_found,stop_found, stop_id,text
...
def demo():
    ...
    #text = ["what is going on with the #2 bus 10 minutes late still no bus",
    #    "the 830 bus route never showed up at 700 N in Provo SB. Or it was extremely late",
    #    "Route 850 bus #9989 was supposed to leave Lehi Station going southbound at 4:51. No driver in site until 5:01. Worried about age."]
   
    with open('hand_crafted_delayed.txt','r', encoding='utf-8') as twitters:
        text = [x.rstrip() for x in twitters.readlines()[:50]]
    with open("out.csv", "w+",encoding='utf-8',newline='') as resultFile:
        wr = csv.writer(resultFile, dialect='excel')
        wr.writerow(['text','timex_found','bus_found','stop_found'])
        for sent in text:
            timex_found,bus_found,stop_found, text = tag(sent)
            wr.writerow([text,timex_found,bus_found,stop_found])
            #print('text:',text)
            #print('timex_found',timex_found)
            #print('bus_found:',bus_found)
            #print('stop_found:',stop_found)
            #print('')